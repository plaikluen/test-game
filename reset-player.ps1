...
Write-Host "[1/4] Deleting auth user by uid (if exists): $PlayerId"
& $firebaseCmd auth:delete $PlayerId --project $ProjectId --force 2>$null | Out-Null

Write-Host "[2/4] Cleaning old auth user by email (if exists): $targetEmail"
...
if ($users) {
  foreach ($u in $users) {
    if ($u.email -eq $targetEmail -and $u.localId -ne $PlayerId) {
      & $firebaseCmd auth:delete $u.localId --project $ProjectId --force 2>$null | Out-Null
    }
  }
}
Remove-Item $tempFile -ErrorAction SilentlyContinue

Write-Host "[3/4] Reading DB entry: $path"
...
Write-Host "[4/4] Resetting DB flag (used=false)"
